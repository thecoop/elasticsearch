...
public final class CentroidAssignment {
    ...
    static <V> void assignSpilled(
        ...
        for (int i = startOrd; i < endOrd; i++) {
            ...
            final IntUnaryOperator centroidOrds;
    ...
    private static <V> int computeSoarAssignment(
        ...
        for (; j < limit; j += 4) {
            ops.soarDistanceBulk(
                vector,
                centroids[centroidOrds.applyAsInt(j)],
                centroids[centroidOrds.applyAsInt(j + 1)],
                centroids[centroidOrds.applyAsInt(j + 2)],
                centroids[centroidOrds.applyAsInt(j + 3)],
                diffs,
                soarLambda,
                vectorCentroidDist,
                distances
            ...
            for (int k = 0; k < distances.length; k++) {
                ...
                if (soar < minSoar) {
                    minSoar = soar;
                    bestAssignment = centroidOrds.applyAsInt(j + k);
                }
            }
        }

        for (; j < centroidCount; j++) {
            int centroidOrd = centroidOrds.applyAsInt(j);
