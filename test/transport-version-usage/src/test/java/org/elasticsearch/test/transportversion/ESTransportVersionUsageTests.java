...
import org.elasticsearch.test.transportversion.ESTransportVersionChecker.WrongLoggerUsage;
...
public class ESTransportVersionUsageTests extends ESTestCase {
        ...
        for (Method method : getClass().getMethods()) {
            if (method.getDeclaringClass().equals(getClass())) {
                if (method.getName().startsWith("check")) {
                    ...
                    ESTransportVersionChecker.check(
                        errors::add,
                        classInputStream,
    ...
    public void testLoggerUsageCheckerCompatibilityWithLog4j2Logger() throws NoSuchMethodException {
        for (Method method : Logger.class.getMethods()) {
            if (ESTransportVersionChecker.LOGGER_METHODS.contains(method.getName())) {
        ...
        for (String methodName : ESTransportVersionChecker.LOGGER_METHODS) {
    ...
    public void checkDeprecationLogger() {
        DeprecationLogger deprecationLogger = DeprecationLogger.getLogger(ESTransportVersionUsageTests.class);
