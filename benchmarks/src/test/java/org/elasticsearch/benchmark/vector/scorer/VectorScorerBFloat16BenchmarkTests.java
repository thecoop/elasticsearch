...
public class VectorScorerBFloat16BenchmarkTests extends ESTestCase {

    private final double delta = 1e-2; // BFloat16 has less precision
    private final VectorSimilarityType function;
    private final int dims;

    public VectorScorerBFloat16BenchmarkTests(VectorSimilarityType function, int dims) {
    ...
    public void testScores() throws Exception {
        for (int i = 0; i < 100; i++) {
            VectorScorerBFloat16Benchmark.VectorData data = new VectorScorerBFloat16Benchmark.VectorData(dims);
            Float expected = null;
            for (var impl : VectorImplementation.values()) {
                var bench = new VectorScorerBFloat16Benchmark();
    ...
    public static Iterable<Object[]> parametersFactory() {
        try {
            String[] dims = VectorScorerBFloat16Benchmark.class.getField("dims").getAnnotationsByType(Param.class)[0].value();
            String[] functions = VectorScorerBFloat16Benchmark.class.getField("function").getAnnotationsByType(Param.class)[0].value();
