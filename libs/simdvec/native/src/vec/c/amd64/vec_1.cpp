...
template<typename Q, __m256(*load_q)(const Q*, int bf16_offset)>
static inline f32_t dotbf16_inner(const bf16_t* d, const Q* q, const int32_t elementCount) {
    constexpr int batches = 4;
    ...
    for (; i < (elementCount & ~(stride - 1)); i += stride) {
        apply_indexed<batches>([&](auto I) {
            sums[I] = _mm256_fmadd_ps(load_bf16(d, i + I * elements), load_q(q, i + I * elements), sums[I]);
        });
    }
...
EXPORT f32_t vec_dotDbf16Qf32(const bf16_t* a, const f32_t* b, const int32_t elementCount) {
    return dotbf16_inner<f32_t, load_f32>(a, b, elementCount);
}
...
EXPORT f32_t vec_dotDbf16Qbf16(const bf16_t* a, const bf16_t* b, const int32_t elementCount) {
    return dotbf16_inner<bf16_t, load_bf16>(a, b, elementCount);
}
